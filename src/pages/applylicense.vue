...
export default {
  ...
  created() {
    this.axios.get('/api/license/getLicense').then((res) => {
      if (res.data === "证书校验成功") {
  ...
  methods: {
    ...
    handleSubmit(name) {
      this.$refs[name].validate((valid) => {
        if (valid) {
          this.loading = true;
          this.axios.post('/api/license/apply',this.licenseForm)
            .then(() => {
              this.title = "证书申请成功,正在安装证书！";
              this.loading = false;
              this.disable = true;
              this.axios.get('/api/install').then(() => {
                this.$router.push('/login');
              });
            })
            .catch(() => {
    ...
    sendSMS() {
      ...
      this.axios.post('/api/sendSMS',this.licenseForm)
        .then((res) => {
          if (res.status == "200") {
            var that = this;
